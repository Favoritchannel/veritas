...
async function rows(cfg) {
  ...
  if (cfg.driver === "sqlite") {
    ...
    const r = db.prepare(cfg.query).all(); db.close(); return r;
  }
  if (cfg.driver === "postgres") {
    const { default: pg } = await import("pg"); const c = new pg.Client({ connectionString: conn }); await c.connect();
    const r = await c.query(cfg.query); await c.end(); return r.rows;
  }
  if (cfg.driver === "mysql") {
    const mysql = await import("mysql2/promise"); const c = await mysql.createConnection(conn);
    const [r] = await c.query(cfg.query); await c.end(); return r;
  }
...
export async function collect(project, cfg) {
  let data;
  try { data = (await rows(cfg)).slice(0, cfg.maxRows || 2000); } catch (e) { project.log(`    database (${cfg.driver}): ${String(e.message).slice(0, 120)} — is the driver installed?`); return []; }
  const asText = (row) => cfg.textColumns?.length ? cfg.textColumns.map((c) => row[c]).filter(Boolean).join(" — ") : Object.entries(row).map(([k, v]) => `${k}: ${v}`).join("; ");
  // Direct-to-facts (no LLM) unless cfg.llm — each row is already structured.
  if (!cfg.llm) return data.map((r) => ({ text: asText(r), confidence: "high", kind: "record", source: { ref: cfg.title || cfg.driver, title: cfg.title || `${cfg.driver} query` } }));
  ...
  for (const ch of chunk(data.map(asText).join("\n"), project.chunkChars)) {
    try {
      ...
      for (const x of d.facts || []) if (x.text) entries.push({ text: x.text, confidence: x.confidence || "medium", kind: "fact", source: { ref: cfg.title || cfg.driver, title: cfg.title || cfg.driver } });
    } catch { /* */ }
  }
  return entries;
}
