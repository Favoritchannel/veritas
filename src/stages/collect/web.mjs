...
export async function collect(project, cfg) {
  ...
  try {
    ...
    project.log(
      "    web: playwright not installed (npm i playwright) — skipping",
    );
    return [];
  }
  ...
  const visionTier = project.config.compute?.vision
    ? project.tier("vision")
    : null;
  ...
  try {
    while (queue.length && seen.size < maxPages) {
      ...
      const d = await page.evaluate(() => ({
        text: (
          document.querySelector("main, article, .content, body")?.innerText ||
          ""
        ).slice(0, 20000),
        title: document.title,
        imgs: [
          ...document.querySelectorAll("main img, article img, .content img"),
        ]
          .map((i) => i.src)
      ...
      try {
        ...
        for (const l of d.links)
          if (l.includes(host) && !seen.has(l) && queue.length < maxPages)
            queue.push(l.split("#")[0]);
      } catch {
        /* */
      }
      // vision on images
        ...
        for (const im of d.imgs.slice(0, 3)) {
          try {
            const r = await vision(
              visionTier,
              "Read this image (chart/table/diagram). Return a compact plain-text description of the data, axes, and any numbers.",
              [im],
            );
