...
function parseFeed(xml) {
  ...
  for (const b of blocks) {
    ...
    const desc = strip((b.match(/<(description|summary|content)[^>]*>([\s\S]*?)<\/\1>/i) || [])[2]);
    const link = strip((b.match(/<link[^>]*>([\s\S]*?)<\/link>/i) || [])[1] || (b.match(/<link[^>]*href="([^"]+)"/i) || [])[1]);
...
export async function collect(project, cfg) {
  ...
  for (const feed of cfg.feeds || []) {
    let xml = ""; try { const r = await fetch(feed, { signal: AbortSignal.timeout(30000), headers: { "user-agent": "veritas" } }); xml = await r.text(); } catch (e) { project.log(`    rss ${feed}: ${e.message}`); continue; }
    const items = parseFeed(xml).slice(0, cfg.maxItems || 40);
    const corpus = items.map((it) => `# ${it.title}\n${it.desc}\n(${it.link})`).join("\n\n");
    for (const ch of chunk(corpus, project.chunkChars)) {
      try {
        ...
        for (const x of d.facts || []) if (x.text) entries.push({ text: x.text, confidence: x.confidence || "medium", kind: "fact", source: { ref: feed, title: cfg.title || feed } });
      } catch { /* */ }
    }
  }
  return entries;
