...
export async function run(project) {
  ...
  if (!raw.length) {
    project.log("consolidate: no raw-entries.json (run collect first)");
    return;
  }
  const seen = new Set(),
    entries = [];
  ...
  for (const [d, arr] of Object.entries(byDomain))
    project.writeOut(`by-domain/${d}.json`, {
      domain: d,
      count: arr.length,
      entries: arr,
    });
  const sources = new Map();
  for (const e of entries) sources.set(e.source?.ref, e.source);
  project.writeOut("sources.json", [...sources.values()].filter(Boolean));
  project.writeOut("consolidated.json", entries);
  project.log(
    `consolidate: ${entries.length} entries (deduped from ${raw.length}) across ${Object.keys(byDomain).length} domains → by-domain/`,
  );
}
