...
export async function run(project) {
  ...
  if (!tier || !ora) {
    project.log(
      "discover: skipped (needs an oracle + analyze tier to cross-check)",
    );
    project.writeOut(
      "NOVEL-FINDINGS.md",
  ...
  await waves(
    domains.map((domain) => async () => {
      ...
      try {
        ...
        project.log(
          `    discover ${domain}: ${String(e.message).slice(0, 60)}`,
        );
      }
    }),
    project.waveWidth,
    (i, t) => project.log(`discover wave ${i}/${t}`),
  );
  ...
  const byd = {};
  for (const f of findings) (byd[f.domain] ||= []).push(f);
  for (const [d, list] of Object.entries(byd)) {
    md += `\n## ${d}\n\n`;
    for (const f of list)
      md += `- **${f.finding}** [${f.confidence}]\n  - mechanism: ${f.mechanism}\n  - why missed: ${f.why_missed}\n  - test: ${f.test}\n`;
  }
  project.writeOut("NOVEL-FINDINGS.md", md);
  project.log(
    `discover: ${findings.length} novel findings → NOVEL-FINDINGS.md`,
  );
}
