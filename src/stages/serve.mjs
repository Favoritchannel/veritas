...
const tokenize = (s) => (String(s).toLowerCase().match(/[a-zа-я0-9]{3,}/gi) || []);
function loadCorpus(project) {
  const p = project.outPath("rag-corpus.jsonl");
  if (!fs.existsSync(p)) throw new Error("no rag-corpus.jsonl — run the build first (rag-pack)");
  const docs = fs.readFileSync(p, "utf-8").split("\n").filter(Boolean).map((l) => JSON.parse(l));
  const df = new Map(); for (const d of docs) for (const t of new Set(tokenize(d.text))) df.set(t, (df.get(t) || 0) + 1);
  return { docs, df, N: docs.length };
}
function retrieve({ docs, df, N }, query, k = 8) {
  const q = tokenize(query);
  const scored = docs.map((d) => { const dt = tokenize(d.text); const set = new Set(dt); let s = 0; for (const t of q) if (set.has(t)) s += Math.log(1 + N / (1 + (df.get(t) || 0))); const boost = d.status === "TRUTH" ? 1.15 : d.status === "CONTRADICTED" ? 0.9 : 1; return { d, s: s * boost }; });
...
async function answer(project, corpus, question) {
  ...
  let tier = null; try { tier = project.tier("serve"); if (!process.env[tier.keyEnv]) tier = null; } catch { tier = null; }
  const context = hits.map((h, i) => `[${i + 1}] (${h.status}${h.sources?.length ? " · " + h.sources[0] : ""}) ${h.text}`).join("\n");
  ...
  const { text } = await chat(tier, sys, `QUESTION: ${question}\n${asData("RETRIEVED FACTS", context)}`, { maxTokens: 1200 });
  return text;
}
