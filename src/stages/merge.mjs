...
export async function run(project) {
  ...
  if (!fs.existsSync(dir)) {
    project.log("merge: run synthesize first");
    return;
  }
  const all = [];
  for (const f of fs.readdirSync(dir).filter((x) => x.endsWith(".json"))) {
    const arr = project.readOut(`facts/${f}`, []);
  ...
  for (const m of all) {
    const k = key(m),
      cur = merged.get(k);
    if (!cur) {
      merged.set(k, { ...m, sources: uniq(m.sources || []) });
      continue;
    }
    if ((CONF_RANK[m.confidence] || 0) > (CONF_RANK[cur.confidence] || 0)) {
      cur.confidence = m.confidence;
      cur.statement = m.statement;
    }
    cur.sources = uniq([...(cur.sources || []), ...(m.sources || [])]);
    cur.breakpoints = uniq([
      ...(cur.breakpoints || []),
      ...(m.breakpoints || []),
    ]);
    ...
    if ((cur.sources || []).length >= 2 && cur.confidence === "medium")
      cur.confidence = "high";
  }
  ...
  for (const m of facts)
    for (const from of m.dependsOn || [])
      for (const to of m.affects && m.affects.length
        ? m.affects
        : [m.statement.slice(0, 48)]) {
        if (!from || !to) continue;
        nodes.set(from.toLowerCase(), {
          id: from.toLowerCase(),
          name: from,
          domain: m.domain,
        });
        nodes.set(String(to).toLowerCase(), {
          id: String(to).toLowerCase(),
          name: String(to).slice(0, 60),
          domain: m.domain,
        });
        edges.push({
          from,
          to: String(to).slice(0, 60),
          relation: m.hidden ? "hidden" : "affects",
          domain: m.domain,
          hidden: !!m.hidden,
        });
      }
  ...
  project.writeOut(
    "hidden.json",
    facts.filter((m) => m.hidden),
  );
  project.writeOut(
    "breakpoints.json",
    facts
      .filter((m) => (m.breakpoints || []).length)
      .map((m) => ({
        domain: m.domain,
  ...
  project.log(
    `merge: ${facts.length} facts, ${edges.length} edges, ${facts.filter((m) => m.hidden).length} hidden → facts.json + dependencies.json`,
  );
}
