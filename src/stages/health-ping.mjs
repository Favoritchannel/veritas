...
async function check(project, corpus) {
  ...
  if (!canaries.length) {
    project.log("health-ping: no canaries configured");
  ...
  for (const q of canaries) {
    ...
    try {
      ...
      ok = text && text.length > 20 && !/no relevant facts/i.test(text);
    } catch (e) {
      text = "ERROR: " + e.message;
    }
    results.push({ q, ok, preview: (text || "").slice(0, 120) });
    project.log(
      `  ${ok ? "✅" : "⚠️"} "${q.slice(0, 50)}" → ${(text || "").slice(0, 70)}`,
    );
  }
  const ok = results.every((r) => r.ok);
  project.writeOut("health-last.json", {
...
export async function run(project, { flags } = { flags: new Set() }) {
  ...
  const tick = async () => {
    const r = await check(project, corpus);
    project.log(`health-ping: ${r.ok ? "healthy ✅" : "DEGRADED ⚠️"}`);
    return r;
  };
  if (once) {
    await tick();
  ...
  project.log(
    `health-ping: looping every ${interval / 60000} min (ctrl-c to stop)`,
  );
}
