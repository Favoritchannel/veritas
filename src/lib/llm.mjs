...
export const asData = (label, text) => `<<<${label}>>>\n${text}\n<<<END ${label}>>>`;
...
export async function chat(tier, system, user, opts = {}) {
  if (!tier || !tier.baseURL || !tier.model) throw new Error("llm: tier missing baseURL/model");
  const key = tierKey(tier);
  const anthropic = tier.compat === "anthropic";
  const url = anthropic ? `${tier.baseURL.replace(/\/$/, "")}/messages` : `${tier.baseURL.replace(/\/$/, "")}/chat/completions`;
  const maxTokens = opts.maxTokens ?? 4000;
  const wantJson = !!opts.json;
  const content = opts.images ? [{ type: "text", text: user }, ...opts.images.map((u) => ({ type: "image_url", image_url: { url: u } }))] : user;
  let body, headers;
  if (anthropic) {
    headers = { "content-type": "application/json", "x-api-key": key, "anthropic-version": "2023-06-01" };
    body = { model: tier.model, max_tokens: maxTokens, system, messages: [{ role: "user", content: opts.images ? content.map((c) => (c.type === "image_url" ? { type: "image", source: { type: "url", url: c.image_url.url } } : c)) : user }] };
  } else {
    headers = { "content-type": "application/json", authorization: `Bearer ${key}`, "HTTP-Referer": "https://github.com/veritas-kb", "X-Title": "veritas" };
    body = { model: tier.model, temperature: opts.temperature ?? 0.2, max_tokens: maxTokens, messages: [{ role: "system", content: system }, { role: "user", content }] };
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    try {
      const r = await fetch(url, { method: "POST", signal: AbortSignal.timeout(90000), headers, body: JSON.stringify(body) });
      const j = await r.json().catch(() => null);
      if (!j || j.error) { lastErr = j?.error?.message || `HTTP ${r.status}`; if (/rate|429|limit|overload|timeout/i.test(lastErr)) continue; throw new Error(lastErr); }
      const text = anthropic ? (j.content?.map((c) => c.text).join("") ?? "") : (j.choices?.[0]?.message?.content ?? "");
...
export async function chatJson(tier, system, user, opts = {}) {
  const { text } = await chat(tier, system, user, { ...opts, json: tier.compat !== "anthropic" });
...
export async function vision(tier, prompt, images, opts = {}) {
  return chat(tier, "", prompt, { ...opts, images, maxTokens: opts.maxTokens ?? 3000 });
}
